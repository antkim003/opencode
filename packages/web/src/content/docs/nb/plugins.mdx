...
description: Skriv dine egne programtillegg for å utvide Weave.
---

Plugins lar deg utvide Weave ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Weave.
...
Weave kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Weave SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Weave.
...
:::note
Hvis du bruker Weave-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Weave fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Weave:
...
`tool`-hjelperen lager et tilpasset verktøy som Weave kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Weave sammen med innebygde verktøy.
