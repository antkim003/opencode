---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Weave.
---

import config from "../../../../config.mjs"
export const console = config.console

Weave bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Weave-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i Weave
konfig.
...
Weave Zen er en liste over modeller levert av Weave-teamet som har vært
testet og verifisert for å fungere godt med Weave. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Weave Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Weave Zen` og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Det fungerer som alle andre leverandører i Weave og er helt valgfritt å bruke.

---

## Weave Go

Weave Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Weave-teamet som har vært
testet og verifisert for å fungere godt med Weave.

1. Kjør kommandoen `/connect` i TUI, velg `Weave Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Slik bruker du Amazon Bedrock med Weave:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Weave:
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Weave skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Weave-konfigurasjonen.
...
for å lagre token i Weave auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Weave bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Weave
...
Slik bruker du GitHub Copilot-abonnementet med Weave:
...
Slik bruker du Google Vertex AI med Weave:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Weave, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver Weave-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Weave-samtale oppført som en separat økt.
...
Du kan konfigurere Weave for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Weave for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Weave for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Weave. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Weave:
...
3. Kopier API-nøkkelen for bruk i Weave.
...
6. **Viktig**: Før du bruker skymodeller i Weave, må du hente modellinformasjonen lokalt:
...
Weave Zen er en liste over testede og verifiserte modeller levert av Weave-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Weave Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Weave Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Weave API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Weave-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Weave-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Weave:
...
Du kan også tilpasse modeller gjennom Weave-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Weave. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
`limit`-feltene lar Weave forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
2. For tilpassede leverandører, sjekk Weave-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Weave-konfigurasjonen.
