---
title: المزوّدون
description: استخدام أي موفّر LLM في Weave.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Weave ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Weave.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Weave.
...
Weave Zen هي قائمة نماذج يوفّرها فريق Weave وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Weave. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Weave Zen.
:::
...
يعمل مثل أي مزوّد آخر في Weave واستخدامه اختياري بالكامل.

---

## Weave Go

Weave Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Weave، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Weave.

1. شغّل الأمر `/connect` في TUI، واختر `Weave Go`، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع Weave:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Weave غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم Weave نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Weave
...
لاستخدام Google Vertex AI مع Weave:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة Weave تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Weave مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Weave. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Weave:
...
3. انسخ مفتاح API لاستخدامه في Weave.
...
6. **مهم**: قبل استخدام نماذج السحابة في Weave، يجب سحب معلومات النموذج محليا:
...
Weave Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Weave. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Weave Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Weave Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Weave.
...
تسمح حقول `limit` لـ Weave بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
