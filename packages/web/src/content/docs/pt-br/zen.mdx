---
title: Zen
description: Lista selecionada de modelos fornecidos pelo Weave.
---
...
O Weave Zen é uma lista de modelos testados e verificados fornecidos pela equipe do Weave.

:::note
O Weave Zen está atualmente em beta.
:::

O Zen funciona como qualquer outro provedor no Weave. Você faz login no Weave Zen e obtém sua chave de API. É **completamente opcional** e você não precisa usá-lo para utilizar o Weave.
...
:::tip
Testamos um grupo selecionado de modelos e provedores que funcionam bem com o Weave.
:::
...
O Weave Zen é um gateway de IA que lhe dá acesso a esses modelos.

---

## Como funciona

O Weave Zen funciona como qualquer outro provedor no Weave.

1. Você faz login no **<a href={console}>Weave Zen</a>**, adiciona seus dados de cobrança e copia sua chave de API.
2. Você executa o comando `/connect` no TUI, seleciona Weave Zen e cola sua chave de API.
...
O [id do modelo](/docs/config/#models) na sua configuração do Weave usa o formato `opencode/<model-id>`. Por exemplo, para GPT 5.2 Codex, você usaria `opencode/gpt-5.2-codex` na sua configuração.
...
- MiniMax M2.5 Free está disponível no Weave por tempo limitado. A equipe está usando esse tempo para coletar feedback e melhorar o modelo.
- Big Pickle é um modelo oculto que está gratuito no Weave por tempo limitado. A equipe está usando esse tempo para coletar feedback e melhorar o modelo.
...
4. Não ter **vinculação** permitindo que você o use com qualquer outro agente de codificação. E sempre permitir que você use qualquer outro provedor com o Weave também.
