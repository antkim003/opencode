---
title: Providers
description: Using any LLM provider in Weave.
---

import config from "../../../config.mjs"
export const console = config.console

Weave uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Weave config.
...
You can customize the providers through the `provider` section in your Weave
config.
...
Weave Zen is a list of models provided by the Weave team that have been
tested and verified to work well with Weave. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Weave Zen.
:::

1. Run the `/connect` command in the TUI, select `Weave Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in Weave and is completely optional to use.

---

## Weave Go

Weave Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Weave team that have been
tested and verified to work well with Weave.

1. Run the `/connect` command in the TUI, select `Weave Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with Weave:
...
Weave. Anthropic explicitly prohibits this.

Previous versions of Weave came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Weave with zero setup:
...
GitLab Duo support in Weave is experimental. Features, configuration, and
behavior may change in future releases.
:::

Weave integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
Weave uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Weave
...
When a `duo-workflow-*` model is selected, Weave will:
...
4. Route tool execution requests through Weave's permission-gated tool system
...
To use Google Vertex AI with Weave:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Weave conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Weave conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for Weave. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with Weave:
...
3. Copy the API key for use in Weave.
...
6. **Important**: Before using cloud models in Weave, you must pull the model information locally:
...
Weave Zen is a list of tested and verified models provided by the Weave team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Weave Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Weave Zen**.

   ```txt
   /connect
   ```

3. Enter your Weave API key.
...
The `limit` fields allow Weave to understand how much context you have left. Standard providers pull these from models.dev automatically.
