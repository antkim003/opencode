---
title: Provider
description: Usa qualsiasi provider LLM in Weave.
---

import config from "../../../../config.mjs"
export const console = config.console

Weave usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Weave.
...
Weave Zen e una lista di modelli forniti dal team Weave, testati e
verificati per funzionare bene con Weave. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Weave Zen.
:::
...
Funziona come qualsiasi altro provider in Weave ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Weave:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Weave non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Weave usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Weave
...
Per usare Google Vertex AI con Weave:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di Weave come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Weave elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per Weave. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con Weave:
...
3. Copia la chiave API da usare in Weave.
...
6. **Importante**: Prima di usare modelli cloud in Weave, devi scaricare le informazioni del modello localmente:
...
Weave Zen è una lista di modelli testati e verificati forniti dal team Weave. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Weave Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Weave Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Weave.
...
I campi `limit` permettono a Weave di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
