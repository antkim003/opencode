---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Weave.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Weave fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Weave avvia la TUI per la directory corrente.
...
Quando usi la TUI di Weave, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Weave. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
