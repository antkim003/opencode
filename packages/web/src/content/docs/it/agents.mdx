...
In Weave esistono due tipi di agenti: agenti primari e subagenti.
...
Weave include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Weave include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Weave include due agenti primari integrati e due subagenti integrati.
...
Se non specifichi una temperature, Weave usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Weave sia per la config specifica del progetto.
...
L'ID modello nella configurazione Weave usa il formato `provider/model-id`. Per esempio, se stai usando [Weave Zen](/docs/zen), useresti `opencode/gpt-5.1-codex` per GPT 5.1 Codex.
