...
Per diagnosticare problemi con Weave, inizia controllando i log e i dati locali che salva su disco.
...
Weave Desktop esegue in background un server locale di Weave (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Weave` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
Weave puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Weave possa ricostruirla.

1. Chiudi completamente Weave Desktop.
...
3. Riavvia Weave Desktop.

---

### Risolvi problemi di connessione al server

Weave Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Su Windows, Weave Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Weave.

---

### Notifiche non visualizzate

Weave Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Weave nelle impostazioni del sistema operativo, e
...
1. Chiudi Weave Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Weave Desktop):
...
Se riscontri problemi con Weave:
