---
title: CLI
description: Opzioni e comandi della CLI di Weave.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di Weave, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
opencode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con Weave in modo programmatico.
...
Avvia l'interfaccia testuale di Weave (interfaccia utente da terminale).
...
La CLI di Weave include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per Weave.
...
Collega un terminale a un backend server di Weave già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
opencode attach [url]
```

Questo consente di usare la TUI con un backend Weave remoto. Per esempio:
...
Weave si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `opencode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Quando Weave si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in Weave.
...
Avvia un server Weave headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.
...
Elenca tutte le sessioni Weave.
...
Mostra statistiche di utilizzo token e costo per le sessioni Weave.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione Weave.

```bash
opencode import <file>
```

Puoi importare da un file locale o da un URL di condivisione Weave.
...
Avvia un server Weave headless con interfaccia web.

```bash
opencode web
```

Avvia un server HTTP e apre un browser per accedere a Weave tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Disinstalla Weave e rimuove tutti i file correlati.
...
Weave può essere configurato tramite variabili d'ambiente.
