---
title: Web
description: En utilisant Weave dans votre navigateur.
---

Weave peut s'exécuter comme une application Web dans votre navigateur, offrant la même expérience puissante de codage d'IA sans avoir besoin d'un terminal.

![Weave Web - Nouvelle session](../../../assets/web/web-homepage-new-session.png)
...
Cela démarre un serveur local sur `127.0.0.1` avec un port disponible aléatoire et ouvre automatiquement Weave dans votre navigateur par défaut.
...
Par défaut, Weave sélectionne un port disponible. Vous pouvez spécifier un port :
...
Par défaut, le serveur se lie à `127.0.0.1` (localhost uniquement). Pour rendre Weave accessible sur votre réseau :

```bash
opencode web --hostname 0.0.0.0
```

Lorsque vous utilisez `0.0.0.0`, Weave affichera les adresses locales et réseau :
...
Une fois démarrée, l'interface web permet d'accéder à vos sessions Weave.
...
![Weave Web - Session active](../../../assets/web/web-homepage-active-session.png)

### Statut du serveur

Cliquez sur "Voir les serveurs" pour afficher les serveurs connectés et leur état.

![Weave Web - Voir Serveurs](../../../assets/web/web-homepage-see-servers.png)
