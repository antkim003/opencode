...
Il existe deux types d'agents dans Weave ; agents primaires et sous-agents.
...
Weave est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Weave est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

Weave est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Si aucune température n'est spécifiée, Weave utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Weave et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Weave utilise le format `provider/model-id`. Par exemple, si vous utilisez [Weave Zen](/docs/zen), vous utiliserez `opencode/gpt-5.1-codex` pour GPT 5.1 Codex.
