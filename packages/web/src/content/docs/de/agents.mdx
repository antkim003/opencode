...
Es gibt zwei Arten von Agenten in Weave; Primäragenten und Subagenten.
...
Weave verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Wenn keine Temperatur angegeben ist, verwendet Weave modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Weave-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Weave-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Weave Zen](/docs/zen) verwenden, würden Sie `opencode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
