...
Možete dodati vanjske alate u Weave koristeći _Model Context Protocol_, ili MCP. Weave podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Weave Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Weave automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Weave će:
...
Ako server zahtijeva autentifikaciju, Weave će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Weave sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Weave, zajedno s ugrađenim alatima. Možete njima upravljati kroz Weave konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Weave sa vašim Sentry nalogom.
