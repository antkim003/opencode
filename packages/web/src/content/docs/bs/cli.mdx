---
title: CLI
description: Weave CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Weave CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
opencode
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa Weave.
...
Pokrenite Weave terminalski korisnički interfejs.
...
Weave CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte Weave agentima.
...
Priključite terminal na već pokrenut Weave backend server pokrenut putem `serve` ili `web` naredbi.

```bash
opencode attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim Weave backend-om. Na primjer:
...
Weave pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `opencode auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Kada se Weave pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u Weave.
...
Pokrenite Weave u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
| `--attach`   |        | Priključite na pokrenuti Weave server (npr. http://localhost:4096)    |
...
Pokrenite Weave headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
opencode serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti Weave-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Prikaži statistiku upotrebe tokena i troškova za vaše Weave sesije.
...
Uvezite podatke sesije iz JSON datoteke ili Weave dijeljenog URL-a.

```bash
opencode import <file>
```

Možete uvesti iz lokalne datoteke ili Weave dijeljenog URL-a.
...
Pokrenite Weave headless server sa web interfejsom.

```bash
opencode web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup Weave-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Deinstalirajte Weave i uklonite sve povezane datoteke.
...
Ažurira Weave na najnoviju verziju ili određenu verziju.
...
Weave CLI prihvata sljedeće globalne zastavice.
...
Weave se može konfigurirati pomoću varijabli okruženja.
