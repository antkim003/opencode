---
title: Proveedores
description: Usando cualquier proveedor LLM en Weave.
---

import config from "../../../../config.mjs"
export const console = config.console

Weave usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Weave.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Weave
configuración.
...
Weave Zen es una lista de modelos proporcionados por el equipo Weave que han sido
probado y verificado para funcionar bien con Weave. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Weave Zen.
:::
...
Funciona como cualquier otro proveedor en Weave y su uso es completamente opcional.

---

## Weave Go

Weave Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Weave que han sido
probado y verificado para funcionar bien con Weave.

1. Ejecute el comando `/connect` en TUI, seleccione `Weave Go` y diríjase a [opencode.ai/auth](https://opencode.ai/zen).
...
Para usar Amazon Bedrock con Weave:
...
:::info
El uso de su suscripción Claude Pro/Max en Weave no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Weave utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Weave
...
Para utilizar Google Vertex AI con Weave:
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Weave enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para Weave. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Weave:
...
3. Copie la clave API para usarla en Weave.
...
6. **Importante**: Antes de usar modelos en la nube en Weave, debe obtener la información del modelo localmente:
...
Weave Zen es una lista de modelos probados y verificados proporcionada por el equipo Weave. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Weave Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Weave Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Weave API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Weave:
...
Los campos `limit` le permiten a Weave comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
