...
Weave Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
Weave Go şu anda beta olarak mevcut.
:::

Go, Weave'daki diğer sağlayıcılar gibi çalışır. Weave Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Weave'u kullanmak için bunu kullanmanıza gerek yoktur.
...
:::tip
Weave ile iyi çalışan seçili bir model ve sağlayıcı grubunu test ettik.
:::
...
Weave Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Weave Go, Weave'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Weave Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Weave Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Weave Go'ya abone olabilir.
:::
...
Weave Go aşağıdaki limitleri içerir:
...
Weave yapılandırmanızdaki [model id](/docs/config/#models) formatı `opencode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.5 için yapılandırmanızda `opencode-go/kimi-k2.5` kullanmalısınız.
...
Weave Go'yu şu amaçlarla oluşturduk:
...
4. Weave ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
