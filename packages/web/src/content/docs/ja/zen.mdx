---
title: Weave Zen
description: Weave によって提供されるモデルの厳選されたリスト。
---
...
Weave Zen は、Weave チームによって提供される、テストおよび検証されたモデルのリストです。

:::note
Weave Zen は現在ベータ版です。
:::
Zen は Weave の他のプロバイダーと同様に機能します。 Weave Zen にログインすると、API キーを取得できます。これは **完全にオプション** であり、Weave を使用するために使用する必要はありません。
...
:::tip
Weave で適切に動作する、厳選されたモデルとプロバイダーのグループをテストしました。
:::
...
Weave Zen は、これらのモデルへのアクセスを可能にする AI ゲートウェイです。

---

## 仕組み

Weave Zen は、Weave の他のプロバイダーと同様に機能します。

1. **<a href={console}>Weave Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、Weave Zen を選択して API キーを貼り付けます。
...
Weave 設定の [model id](/docs/config/#models)
...
- MiniMax M2.5 Free は期間限定で Weave で利用可能です。チームはこの期間を利用してフィードバックを収集し、モデルを改善します。
- Big Pickle は、期間限定で Weave で無料で利用できるステルスモデルです。チームはこの期間を利用してフィードバックを収集し、モデルを改善します。
...
4. 他のコーディングエージェントとの併用を許可することで、**ロックイン**がなくなります。また、常に Weave で他のプロバイダーも使用できるようにします。
